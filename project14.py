...
def extract_text_from_license_plate(license_plate_image):
    ...
    try:
        ...
        configs = [
            ...
            ('--oem 1 --psm 7 -c tessedit_char_whitelist=0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ가나다라마바사아자차카타파하', 'kor+eng'),
            ('--oem 1 --psm 6', 'kor+eng'),
            ('--oem 1 --psm 8', 'eng'),
            ('--oem 3 --psm 8', 'eng')
        ]
                ...
                if len(text_clean) >= 4:
                    ...
                    for pattern in korean_patterns:
                        if re.match(pattern, text_clean):
                            print(f"OCR Success: {text_clean} (Pattern matched)")
                            ocr_debug_info = f"Success: {text_clean}"
                            return text_clean
                    
                    # 영문+숫자 조합
                    if len(text_clean) >= 4 and re.match(r'^[A-Z0-9]+$', text_clean):
                        print(f"OCR Success: {text_clean} (English plate)")
                        ocr_debug_info = f"Success: {text_clean}"
                        return text_clean
...
def index():
    ...
    <html>
    <head>
        <title>Smart Parking System - Optimized</title>
        <style>
            ...
            .optimized { background-color: #d1ecf1; border: 1px solid #bee5eb; color: #0c5460; }
        </style>
    </head>
    <body>
        <div class="container">
            <h1>스마트 주차 관리 시스템 (최적화 버전)</h1>
            
            <div class="optimized">
                <h4>최적화 완료</h4>
                <p>신뢰도 임계값: 0.7 (확실한 감지만 처리)</p>
                <p>처리 대상: 'plat' 클래스만</p>
                <p>OCR 정확도 향상을 위한 전처리 강화</p>
            </div>
