...
def generate_frames():
    ...
    print("Picamera2 + EasyOCR + HiveMQ Cloud video stream started")
    
    while True:
        try:
            # Picamera2에서 프레임 캡처 (검색 결과 참고)
            ...
            # Picamera2 + EasyOCR 표시
            ocr_status = "Picamera2+EasyOCR (KO+EN)" if korean_support else "Picamera2+EasyOCR (EN)"
...
def index():
    ...
    <html>
    <head>
        <title>Smart Parking System - Picamera2 EasyOCR HiveMQ Cloud (paho-mqtt 2.x)</title>
        <style>
    ...
    <body>
        <div class="container">
            <h1>스마트 주차 관리 시스템 (Picamera2 + EasyOCR + HiveMQ Cloud)</h1>
            
            <div class="picamera-mode">
                <h4>Picamera2 + EasyOCR + HiveMQ Cloud MQTT (paho-mqtt 2.x 호환)</h4>
            ...
            <div class="video-container">
                <img src="{{{{ url_for('video_feed') }}}}" width="640" height="480" alt="Picamera2 + EasyOCR + HiveMQ Feed">
            </div>
...
if __name__ == '__main__':
    try:
        print("스마트 주차 시스템 시작 (Picamera2 + EasyOCR + HiveMQ Cloud + paho-mqtt 2.x)!")
